...
package com.am.widget.wheelview;
...
import com.am.widget.wheelview.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	...
	private boolean centerDiff = false;
	...
	private void initResourcesIfNecessary() {
		...
		if (centerDrawable == null) {
			GradientDrawable drawable = new GradientDrawable(
					...
					0x10d2483b, 0x70d2483b, 0x70d2483b });

			drawable.setStroke(width, 0xe0d2483b);
			centerDrawable = drawable;
		}

		if (topShadow == null) {
			topShadow = new GradientDrawable(Orientation.TOP_BOTTOM, new int[] {
					0xFF640900, 0x00AAAAAA, 0x00AAAAAA });
		}

		if (bottomShadow == null) {
			bottomShadow = new GradientDrawable(Orientation.BOTTOM_TOP,
					new int[] { 0xFF640900, 0x00AAAAAA, 0x00AAAAAA });
		}

		if (getBackground() == null) {
			int inset = (int) (getResources().getDisplayMetrics().density * 4);
			Drawable[] array = new Drawable[2];
			GradientDrawable drawable = new GradientDrawable(
					GradientDrawable.Orientation.TOP_BOTTOM, new int[] {
					0xe0d2483b, 0x40d2483b, 0xe0d2483b });
			drawable.setStroke(width, 0xe0d2483b);
			array[0] = drawable;
			array[1] = new GradientDrawable(
					GradientDrawable.Orientation.TOP_BOTTOM, new int[] {
					0xfffeecea, 0xffffffff, 0xfffeecea });
			LayerDrawable ld = new LayerDrawable(array);
			ld.setLayerInset(1, inset, width, inset, width);
	...
	private boolean rebuildItems() {
		...
		if (centerDiff) {
			for (int i = 0; i < itemsLayout.getChildCount(); i++) {
				makeViewDiff(itemsLayout.getChildAt(i), 0x7a000000);
			}
			int itemOffset = currentItem - firstItem;
			if (itemOffset >= 0 && itemsLayout.getChildCount() > itemOffset) {
				makeViewDiff(itemsLayout.getChildAt(itemOffset), Color.WHITE);
			}
		}
		return updated;
	}
